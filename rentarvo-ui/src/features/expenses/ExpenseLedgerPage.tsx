...
export function ExpenseLedgerPage() {
  ...
  const bulkDeleteMutation = useMutation({
    mutationFn: async () => {
      await Promise.all(Array.from(selected).map((id) => api.delete(`/expenses/${id}`)));
    },
    onSuccess: () => {
  ...
  return (
    <div className="p-4 lg:p-6 space-y-4">
              ...
              <tbody>
                {filtered.map((t: any) => (
                  <tr key={t.id} className={`border-t hover:bg-gray-50 ${selected.has(t.id) ? 'bg-brand-50/50' : ''}`}>
                    ...
                    <td className="px-4 py-3">{formatDate(t.expenseDate)}</td>
                    ...
                    <td className="px-4 py-3 text-gray-500">
                      {t.contact?.organization || t.contact?.fullName || '\u2014'}
                    </td>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-xs text-gray-400">{formatDate(t.expenseDate)} · {t.category?.name}</p>
                    </div>
                    <p className="font-semibold tabular-nums text-red-600 shrink-0 ml-2">{formatMoney(t.amount)}</p>
                  </div>
                </div>
                <div className="flex items-center gap-0.5 shrink-0">
                  <button onClick={() => { setEditData(t); setShowForm(true); }} className="p-1.5 text-gray-400 hover:text-brand-600">
                    <Pencil size={14} />
                  </button>
                  <button onClick={() => setDeleteTarget(t.id)} className="p-1.5 text-gray-400 hover:text-red-500">
                    <Trash2 size={14} />
                  </button>
                </div>
              </div>
      ...
      <ConfirmDialog
        ...
        message="This will permanently delete this expense transaction. This action cannot be undone."
