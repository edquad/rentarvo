...
export function RecordExpenseModal({ onClose, editData }: Props) {
  ...
  // In edit mode, don't filter by entity so existing property always appears in dropdown
  ...
  const { data: categories = [] } = useQuery({
    ...
    queryFn: () => api.get<any[]>('/categories?kind=EXPENSE'),
  });
  ...
  }, [isEdit, properties.length]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const mutation = useMutation({
    mutationFn: (data: ExpenseFormData) => {
      ...
      return isEdit
        ? api.put(`/expenses/${editData.id}`, cleanPayload(data))
        : api.post('/expenses', cleanPayload(data));
    },
    onSuccess: () => {
  ...
  return (
    <div ref={focusTrapRef} className="fixed inset-0 z-50 flex items-center justify-center" onKeyDown={(e) => { if (e.key === 'Escape') onClose(); focusTrapHandleKeyDown(e); }} role="dialog" aria-modal="true" aria-labelledby="expense-modal-title">
      ...
      <div className="relative bg-white rounded-2xl p-6 w-full max-w-md mx-4 z-10 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <h2 id="expense-modal-title" className="text-lg font-semibold">{isEdit ? 'Edit Expense' : 'Record Expense'}</h2>
        ...
        <form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="space-y-4">
          ...
          <div>
            ...
            <select {...register('contactId')} className="w-full px-3 py-2.5 border rounded-lg focus:ring-2 focus:ring-brand-500 outline-none bg-white">
              <option value="">—</option>
              {contacts.map((c: any) => (
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="date" {...register('expenseDate')} className="w-full px-3 py-2.5 border rounded-lg focus:ring-2 focus:ring-brand-500 outline-none" />
          ...
          >
            {mutation.isPending ? (isEdit ? 'Saving...' : 'Recording...') : (isEdit ? 'Save Changes' : 'Record Expense')}
          </button>
        </form>
      </div>
    </div>
